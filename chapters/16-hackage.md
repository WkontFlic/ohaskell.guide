...
Hackage &mdash; это центральный репозиторий Haskell-библиотек, или, как принято у нас называть, пакетов (англ. package). Название репозитория происходит от слияния слов `Haskell` и `package`. Hackage существует с 2008 года и живёт [здесь](https://hackage.haskell.org/). Ранее упомянутая стандартная библиотека тоже живёт в Hackage и называется она `base`. Каждой библиотеке выделена своя страница.

Каждый из Hackage-пакетов живёт по адресу, сформированному по неизменной схеме: `https://hackage.haskell.org/package/ИМЯ_ПАКЕТА`. Так, дом стандартной библиотеки &mdash; `http://hackage.haskell.org/package/base`. Hackage &mdash; открытый репозиторий: любой разработчик может добавить туда свои пакеты.
...
Hackage весьма большой, поэтому искать пакеты можно 2-я способами:

- на [единой странице всех пакетов](https://hackage.haskell.org/packages/). Здесь перечислены все пакеты, а для нашего удобства они расположены по тематическим категориям.

- через специальный поисковик [Hoogle](https://hoogle.haskell.org/)

Этот поисковик просматривают внутренности Hackage и индексирует их, и вы будете часто им пользоваться. Пользуемся им как обычным поисковиком: 
например, знаем мы имя функции, а в каком пакете/модуле она находится &mdash; забыли. Вбиваем в поиск имя функции &mdash; получаем имя пакета/модуля в котором она прописана.
...
в имени модуля отражена иерархия пакета. `Data.Char` означает, что внутри пакета `base` есть каталог `Data`, внутри которого есть файл `Char.hs`, открыв который, мы увидим:
...
Соответственно, имена наших собственных модулей тоже отражают место, в котором они находятся. Так, один из модулей в моём рабочем проекте носит название `Common.Performers.Click`. Это означает, что живёт этот модуль здесь: `src/Common/Performers/Click.hs`.
...
Импорт модуля `Data.Char` делает доступным для нас всё то, что включено в интерфейс этого модуля (функции модуля которые доступны снаружи модуля). Откроем наш собственный модуль `Lib`:

```haskell
module Lib      -- интерфейс модуля Lib
    ( someFunc  -- функция someFunc - доступна снаружи модуля
    ) where

someFunc :: IO ()       -- Объявление функции someFunc
someFunc = putStrLn "someFunc"
```

Имя функции `someFunc` упомянуто в интерфейсе модуля, а именно между круглыми скобками, следующими за именем модуля. Немного переформатируем скобки:
...
main :: IO ()
main = putStrLn . anotherFunc $ "Привет"
```
...
тогда функция `anotherFunc` тоже станет видимой всему миру. Механизм интерфейса модуля позволяет нам разделять на:
- функции которые модуль предоставляет внешним пользователям
- внутренние функции которые предназначенны для использования внутри модуля другими функциями.
...
Вспомним функцию `putStrLn`: она существует не только в подключённом по умолчанию модуле `Prelude`, но и в модуле `Data.Text.IO` из пакета `text`:
...
1. Можно указать принадлежность функции конкретному модулю. Из сообщения об ошибке уже видно, как это можно сделать:
...
2. Слишком длинно, нужно укоротить. Импортируем модуль под коротким именем:
...
3. Иногда, для большего порядка, используют qualified-импорт:
...
```haskell
import Data.Char
import Data.Text (pack)  -- импортируем только функцию pack!

main :: IO ()
main = putStrLn $ map toUpper "haskell.org"
```

Мы подразумеваем стандартную функцию `map`, однако в модуле `Data.Text` тоже содержится функция по имени `map`. К счастью, никакого конфликта не будет, ведь мы импортировали не всё содержимое модуля `Data.Text`, а лишь одну его функцию `pack`:
...
Если же мы хотим импортировать 2 или более функции, перечисляем их через запятую:

```haskell
import Data.Text (pack, unpack)
```

Существует и прямо противоположный путь: вместо выборочного включения &mdash; выборочное выключение. Избежать конфликта между функциями `putStrLn` можно было бы и так:

```haskell
import Data.Text.IO hiding (putStrLn) -- hiding - запрещает импортировать функцию putStrLn

main :: IO ()
main = putStrLn ...  -- Сомнений нет: из модуля Prelude
```
