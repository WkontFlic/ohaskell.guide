...
```haskell
let ДЕКЛАРАЦИЯ in ВЫРАЖЕНИЕ
```

где `ДЕКЛАРАЦИЯ` &mdash; выражения, декларируемые нами, а `ВЫРАЖЕНИЕ` &mdash; выражение, в котором используется выражения из `ДЕКЛАРАЦИЯ`. Когда мы написали:
