...
В этой главе мы познакомимся с кортежем и ещё ближе подружимся с сопосовлением по образцу/шаблону (паттерн матчингом) с которым познакомилисьв главе 8 Выбор и образцы.
...
main =
  let пара  = создатьПсевдоним "173.194.71.106" "www.ya.ru" -- создаём локальный кортеж "пара" 
      host  = fst пара  -- Берём первое значение пары - "173.194.71.106"
      alias = snd пара  -- Берём второе значение пары - "www.ya.ru"
...
```haskell
let (хост, alias) = создатьПсевдоним "173.194.71.106" "www.ya.ru"

let (хост, alias) = ("173.194.71.106", "www.ya.ru")
